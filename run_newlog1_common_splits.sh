...
ROOT="${ROOT:-${DATA_ROOT:-$DEFAULT_WORK_ROOT/data/prepared_common_splits_powerlaw_abc}}"
ROOT="${ROOT%/}"
TNS_ROOT="${TNS_ROOT:-${ROOT}_tns}"
TNS_ROOT="${TNS_ROOT%/}"
OUT_ROOT="${OUT_ROOT:-$DEFAULT_WORK_ROOT/logs/newlog1}"
OUT_ROOT="${OUT_ROOT%/}"
OUT_PARENT="$(dirname "$OUT_ROOT")"
WORK_BASE="$(dirname "$OUT_PARENT")"
if [[ "$WORK_BASE" == "/" ]]; then
  DEFAULT_WORK_TMP="/tmp/newlog1_common_splits"
else
  DEFAULT_WORK_TMP="$WORK_BASE/tmp/newlog1_common_splits"
fi
...
RUN_CUTC_CCD="${RUN_CUTC_CCD:-1}"
RUN_CUTC_ALS="${RUN_CUTC_ALS:-1}"
RUN_BLCO="${RUN_BLCO:-1}"
...
run_cutc() {
  ...
  if [[ "$RUN_CUTC_SGD" == "1" ]]; then
    ...
    echo "[skip] CUTC-sgd disabled for $ds"
  fi
