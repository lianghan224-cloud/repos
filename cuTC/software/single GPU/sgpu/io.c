    ...
    idx_t* offset)
{
  ...
  idx_t m = 0;
  idx_t ind;
  idx_t shape_nmodes = 0;
  idx_t shape_dims[MAX_NMODES];
  for(m=0; m < MAX_NMODES; ++m) {
    shape_dims[m] = 0;
  }
  ...
  while((read = getline(&line, &len, fin)) != -1) {
    if(read > 1 && line[0] == '#') {
      if(strncmp(line, "# shape", 7) == 0 || strncmp(line, "#shape", 6) == 0) {
        ptr = line + ((line[1] == ' ') ? 7 : 6);
        shape_nmodes = 0;
        while(shape_nmodes < MAX_NMODES) {
          char* endptr = NULL;
          idx_t dim = strtoull(ptr, &endptr, 10);
          if(endptr == ptr) {
            break;
          }
          shape_dims[shape_nmodes++] = dim;
          ptr = endptr;
        }
      }
      continue;
  ...
  for(m=0; m < nmodes; ++m) {
    outdims[m] = (shape_nmodes > 0) ? shape_dims[m] : 0;
    offset[m] = 1;
  }
  ...
  *outnmodes = nmodes;
  ...
  /* adjust dims when zero-indexing. If a # shape comment is present, it is
   * already the full tensor shape and must not be incremented. */
  for(m=0; m < nmodes; ++m) {
    if(shape_nmodes == 0 && offset[m] == 0) {
      ++outdims[m];
    }
  }
