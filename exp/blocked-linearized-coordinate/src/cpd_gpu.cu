...
static long long walltime_us_now() {
    struct timeval tv;
...
static EvalMetrics evaluate_sparse_tensor(const SparseTensor* X, const KruskalModel* A) {
    ...
    out.mae = 0.0;
    out.er = 0.0;
    if (!X || X->nnz == 0) return out;

    const int nmodes = X->nmodes;
    const IType rank = A->rank;
    double loss = 0.0;
    double mae = 0.0;
    double st2 = 0.0;

    #pragma omp parallel for reduction(+:loss,mae,st2)
    for (IType x = 0; x < X->nnz; ++x) {
        double estimate = 0.0;
        for (IType r = 0; r < rank; ++r) {
            double prod = (A->lambda ? (double)A->lambda[r] : 1.0);
            for (int m = 0; m < nmodes; ++m) {
                const IType idx = X->cidx[m][x];
                prod *= (double)A->U[m][idx * rank + r];
            }
            estimate += prod;
        }
        const double truth = (double)X->vals[x];
        const double err = truth - estimate;
        loss += err * err;
        mae += fabs(err);
        st2 += truth * truth;
    }
    ...
    out.loss = loss;
    out.rmse = sqrt(loss / (double)X->nnz);
    out.mae = mae / (double)X->nnz;
    out.er = (st2 > 0.0) ? sqrt(loss / st2) : 0.0;
    return out;
}

}  // namespace

FType cpals_blco_dev(blcotensor* at_host, KruskalModel* A, int maxiter, FType tolerance, int kernel, bool stream_data, bool do_batching, int thread_cf, int nprtn, const SparseTensor* train_set, const SparseTensor* val_set, const SparseTensor* holdout_set, long long project_start_us) {
    ...
    if (project_start_us <= 0) {
        project_start_us = walltime_us_now();
    }

    printf("Begin CP-ALS decomposition, rank=%d maxiter=%d tolerance=%f\n", R, maxiter, tolerance);

    // Epoch 0 metrics before any update.
    for (IType i = 0; i < N; ++i) {
        check_cuda(cudaMemcpy(A->U[i], M_dev->U[i], sizeof(FType) * R * at_host->modes[i], cudaMemcpyDeviceToHost), "cudaMemcpy mats to host");
    }
    check_cuda(cudaMemcpy(A->lambda, M_dev->lambda, sizeof(FType) * R, cudaMemcpyDeviceToHost), "cudaMemcpy lambda to host");

    EvalMetrics train_m = evaluate_sparse_tensor(train_set, A);
    EvalMetrics val_m = evaluate_sparse_tensor(val_set, A);
    ...
    for (IType epoch = 1; epoch <= maxiter; ++epoch) {
        ...
        train_m = evaluate_sparse_tensor(train_set, A);
        val_m = evaluate_sparse_tensor(val_set, A);
        holdout_m = evaluate_sparse_tensor(holdout_set, A);

        total_time_us = walltime_us_now() - project_start_us;
        if (total_time_us < 0) total_time_us = 0;

        printf("epoch:%d   loss: %0.5e   RMSE-tr: %0.5e   RMSE-vl: %0.5e   RMSE-u: %0.5e   MAE-tr: %0.5e   MAE-vl: %0.5e   MAE-u: %0.5e   ER-tr: %0.5e   ER-vl: %0.5e   ER-u: %0.5e \n",
               (int)epoch, train_m.loss, train_m.rmse, val_m.rmse, holdout_m.rmse, train_m.mae, val_m.mae, holdout_m.mae, train_m.er, val_m.er, holdout_m.er);
    ...
    printf("TOTAL_TIME_us:%lld (%.3f s)   TOTAL_GPU_us:%lld (%.3f s)\n",
           total_time_us, (double)total_time_us / 1e6, total_gpu_time_us, (double)total_gpu_time_us / 1e6);
